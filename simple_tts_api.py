"""
Simple TTS API implementation using gTTS.
This script provides a simple API for text-to-speech conversion using gTTS.
"""
...
def text_to_speech_stream(text, language="en", slow=False, emotion="neutral"):
    """
...
def text_to_speech_base64(text, language="en", slow=False, emotion="neutral"):
    """
    Convert text to speech and return the audio data as a base64-encoded string.
    ...
    """
    audio_data = text_to_speech_stream(text, language, slow, emotion)
    if audio_data:
...
if __name__ == "__main__":
    # Test text
    test_text = "Hello, this is a test of the text to speech API with human-like voice enhancements."
    
    # Test different emotions
    emotions = ["neutral", "friendly", "professional", "enthusiastic"]
    
    for emotion in emotions:
        output_file = f"output_{emotion}.mp3"
        print(f"Generating speech with {emotion} emotion...")
        success = text_to_speech(test_text, output_file=output_file, emotion=emotion)
